...
\subsection{Concurrency Stress Testing and Architectural Resilience}
To validate production readiness, the FastAPI endpoints were subjected to stress testing utilizing the Locust framework at 1,000 concurrent users, simulating a mix of continuous fleet telemetry requests and on-demand LLM diagnostic requests. The API was run with 4 Uvicorn worker processes (\texttt{uvicorn main:app --workers 4}) to give the synchronous TensorFlow inference calls process-level parallelism rather than contending for a single event loop. Across 4,093 total requests spanning the health-check, prediction, and analysis endpoints, 463 failures were recorded, all isolated to the \texttt{/analyze} endpoint (an endpoint-level failure rate driven by third-party Azure OpenAI API response latency exceeding the test's timeout threshold under concurrency), resulting in an overall failure rate of approximately 11.3\% across all endpoints combined, with a strict \textbf{0\% failure rate} on both core ML inference paths (\texttt{/predict}, 1,177 requests, and the fleet-simulated \texttt{/predict} variant, 1,871 requests).

During load testing, response latency on the inference endpoints rose substantially at this concurrency (median 50--65 seconds, 99th percentile up to 139 seconds), reflecting genuine CPU contention across the 4 worker processes at 1,000 simultaneous users rather than an absence of load; the zero-failure result on \texttt{/predict} confirms every request completed and returned correctly under this load rather than timing out, which is the property being validated here, not low latency at this scale. The architectural implementation of \texttt{asyncio.to\_thread()} combined with the multi-worker deployment prevented event-loop blocking and request-level failures throughout. The failures observed were exclusively isolated to the \texttt{/analyze} endpoint's dependency on third-party Azure OpenAI API latency, which is independent of the system's own streaming architecture.

This confirms that the asynchronous, multi-worker streaming architecture is resilient under high concurrency and structured for horizontal scaling across cloud-based Kubernetes clusters in a production environment. Reducing per-request latency at this scale, for example via containerized horizontal scaling beyond 4 local worker processes, is discussed as future work in Section~V.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{locust_test_results.png}}
\caption{Locust Load Test statistics at 1,000 concurrent users with 4 Uvicorn workers. The metrics validate a 0\% failure rate on both core ML inference endpoints, with failures strictly isolated to the third-party LLM API-dependent endpoint.}
\label{fig:locust_results}
\end{figure}
...
The system has already been validated at 1,000 concurrent users with a 0\% failure rate on both core inference endpoints, as reported in Section~IV. Future iterations of this research will extend concurrency testing beyond that already-validated 1,000-user baseline, for example to 5,000 or 10,000 concurrent users, to identify the precise queue-saturation threshold under \texttt{asyncio.to\_thread()} offloading combined with multi-worker deployment. Additional future work includes exploring edge-computing deployments that embed quantized versions of the inference models directly onto the vehicle's onboard microcontrollers, and investigating whether increasing per-battery cycle coverage (e.g., via data augmentation or a larger source dataset) can extend windowed test evaluation beyond the current 13 of 34 batteries.
